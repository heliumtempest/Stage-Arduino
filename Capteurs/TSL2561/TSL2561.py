...
class Capteur():
    ...
    bdd = None  # Connexion vers la base de données
    t0 = datetime.now()  # Timestamp
    requete = "INSERT INTO TSL2561(\"Lux\", \"LuminositeIR\", \"Luminosite\", \"DateMesure\") " \
              "VALUES ({L}, {IR}, {Lum}, {ts})"
    # Requête SQL (à formater) pour ajouter un enregistrement dans la base de données
    # Faut des " autour des noms de colonnes dans la requête SQL (cf. le script de création de la table)

    # Nom du fichier .csv à créer (nom capteur + date et heure du jour)
    chemin_csv = "Capteurs/TSL2561/csv/TSL2561_" + datetime.strftime(t0, '%Y-%m-%d_%H-%M-%S') + ".csv"
    ...
    # Écriture du header :
    csv = open(chemin_csv, 'a')
    csv.write("Lux;LumIR;Lum;Tec\n")
    csv.close()
